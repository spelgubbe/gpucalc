...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if(!array_diff($postKeys, array_keys($_POST)) && array_search('', $_POST) === false){
		...
		$texelFillRate = calcTFill($_POST[$postKeys[0]], $_POST[$postKeys[1]]);

		/* Echo result unless it's false, else error message */
		echo $texelFillRate ? formatOutput($texelFillRate/$unitDivs[$unit], 3) . ' ' . $unit : 'Amount of Texture Mapping Units (TMUs) needs to be an integer.';
	} else{
